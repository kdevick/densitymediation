...
c
c     Copyright: Alejandro Jara, 2008 - 2017.
c
c     Version 4.0: 
c
c     Last modification: 19-01-2017.
c     
c     Changes and Bug fixes:
c
c     Version 4.0 to Version 5.0:
c          - A function to extract the samples of the conditional
c            densities and cdc is included.
c
...
c=======================================================================
      subroutine readlddpdens2(nsave,npred,ngrid,denspw,denssam)
c=======================================================================
      implicit none
...
c++++ working
      real*8 denspw(npred,ngrid)

c++++ output
      real*8 denssam(nsave,npred*ngrid)
...
c+++++++ check if the user has requested an interrupt
         ...
         count=0
         do i=1,npred
            read(1) (denspw(i,j),j=1,ngrid)
            do k=1,ngrid
               count=count+1
               denssam(ii,count)=denspw(i,k)
            end do
         end do
      end do
...
c=======================================================================
      subroutine readlddpcdf2(nsave,npred,ngrid,cdfpw,cdfsam)
c=======================================================================
      implicit none
...
c++++ working
      real*8 cdfpw(npred,ngrid)

c++++ output
      real*8 cdfsam(nsave,npred*ngrid)
...
c+++++++ check if the user has requested an interrupt
         ...
         count=0
         do i=1,npred
            read(3) (cdfpw(i,j),j=1,ngrid)
            do k=1,ngrid
               count=count+1
               cdfsam(ii,count)=cdfpw(i,k)
            end do
         end do
      end do
